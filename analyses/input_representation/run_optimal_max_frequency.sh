...
PREPROCESS_STFT_MAX_FREQUENCY_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#preprocess_type[@]} % ${#preprocess_stft_max_frequency[@]} ))
SPLITS_TYPE_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#preprocess_type[@]} / ${#preprocess_stft_max_frequency[@]} % ${#splits_type[@]} ))
CLASSIFIER_TYPE_IDX=$(( ($SLURM_ARRAY_TASK_ID-1) / ${#eval_names[@]} / ${#subjects[@]} / ${#preprocess_type[@]} / ${#preprocess_stft_max_frequency[@]} / ${#splits_type[@]} % ${#classifier_type[@]} ))
...
for PREPROCESS_STFT_NPERSEG_IDX in "${!preprocess_stft_nperseg[@]}"; do
    ...
    for PREPROCESS_STFT_WINDOW_IDX in "${!preprocess_stft_window[@]}"; do
        ...
        echo "Running eval for eval $EVAL_NAME, subject $SUBJECT, trial $TRIAL, preprocess $PREPROCESS_TYPE, nperseg $PREPROCESS_STFT_NPERSEG, max_frequency $PREPROCESS_STFT_MAX_FREQUENCY, window $PREPROCESS_STFT_WINDOW, classifier $CLASSIFIER_TYPE"
